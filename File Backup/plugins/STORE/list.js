  ...
  try {
    ...
    if (searchResult.length === 0) {
      ...
      if (media) {
        ...
        if (buffer) {
          await sendMediaMessage(sock, remoteJid, media, buffer, text, message);
        } else {
...
function getMediaKind(mediaFileName) {
...
async function sendMediaMessage(sock, remoteJid, mediaFileName, buffer, caption, quoted) {
  try {
    const mediaKind = getMediaKind(mediaFileName);
    ...
    if (mediaKind === "sticker") {
      ...
      if (caption) {
        await sock.sendMessage(remoteJid, { text: caption }, { quoted });
      }
      return;
    }

    if (mediaKind === "audio") {
      await sock.sendMessage(remoteJid, { audio: buffer, mimetype: "audio/mpeg" }, { quoted });
      if (caption) {
        await sock.sendMessage(remoteJid, { text: caption }, { quoted });
      }
      return;
